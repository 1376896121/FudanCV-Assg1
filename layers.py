...
class Linear():
    def __init__(self, shape, requires_grad=True, bias=True, lambda_reg=0.1, **kwargs):
        '''
    ...
    def backward(self, eta):
        ...
        if self.require_grad:
            ...
            # 计算权重梯度并加上L2正则化项
            self.W.grad = np.dot(self.x.T, eta) / batch_size + self.lambda_reg * self.W.data
...
class Sigmoid():
    ...
    def backward(self, eta):
        return self.y * (1 - self.y) * eta


class Tanh():
    def forward(self, x):
        # 防止overflow, 要避免计算exp(x)
        self.y = 2/ (1 + np.exp(-2*x)) - 1
        return self.y

    def backward(self, eta):
        return (1 + self.y) * (1 - self.y) * eta
